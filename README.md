...
## به‌روزرسانی‌های اخیر (امنیت پنل ادمین + بازطراحی کامل mobile-first + سئو)
- **رخنه‌ی امنیتی جدی رفع شد:** پنل ادمین قبلاً فقط چک می‌کرد کاربر لاگین کرده یا نه — یعنی هر بازدیدکننده‌ای که از `login.html` تو سایت اصلی ثبت‌نام می‌کرد، می‌تونست وارد `dashboard.html` هم بشه! الان بعد از لاگین و همچنین در ابتدای `initDashboard()` (لایه‌ی دوم، جدا از هم)، سند `admins/{uid}` چک می‌شه؛ اگه نبود فوراً signOut می‌شه. **نیاز به تغییر Rule داره** (`admins/{uid}` باید `allow get` برای خود کاربر داشته باشه، نه `read:false` مطلق) — بخش «قوانین امنیتی» رو دوباره Publish کن.
- **بازطراحی کامل mobile-first:** تقریباً کل `style.css` از الگوی «دسکتاپ + max-width» به «موبایل پایه + min-width» تبدیل شد. جزئیات کامل و نتیجه‌ی تست‌ها پایین‌تره.
- گرید فیلم‌ها/سریال‌ها زیر ۴۸۰px حالا دقیقاً ۲ ستونه (قبلاً به یک ستون غول‌پیکر می‌افتاد).
- هدر بالا در موبایل فقط لوگو + دکمه‌حساب‌کاربریه؛ جستجو از هدر حذف و به‌جاش آیتم «جستجو» به bottom-nav اضافه شد (۵ آیتم: خانه/فیلم‌ها/سریال‌ها/علاقه/جستجو).
- همه دکمه‌ها/آیکون‌های قابل‌لمس حداقل ۴۴×۴۴px شدن (استاندارد لمسی).
- لینک‌های دانلود، نوار فیلترها، و فرم‌های پنل ادمین زیر یک بریک‌پوینت مشخص تمام‌عرض/زیر‌هم می‌چینن، نه فشرده کنار هم.
- سئو: `<h1>` که تو ۴ صفحه اصلی اصلاً وجود نداشت اضافه شد؛ پوسترها از CSS background به `<img alt="...">` واقعی تبدیل شدن (قبلاً برای گوگل/screen reader کاملاً نامرئی بودن)؛ `robots.txt`، `sitemap.xml`، فاوآیکون، و JSON-LD اضافه شد.

## به‌روزرسانی‌های قبلی (بازطراحی + نظرات/دانلود + لاگین گوگل)
...
## نتیجه‌ی تست واقعی mobile-first (Playwright، نه حدس)
تست شد روی یه هارنس با مارک‌آپ واقعی سایت (بدون فایربیس، چون این محیط به اینترنت دسترسی نداره) در ۴ عرض خواسته‌شده:

| عرض | Overflow افقی | ستون‌های Grid | نتیجه |
|---|---|---|---|
| 360px | ندارد ✅ | ۲ ستون ✅ | تمیز |
| 390px | ندارد ✅ | ۲ ستون ✅ | تمیز |
| 414px | ندارد ✅ | ۲ ستون ✅ | تمیز |
| 768px | **یک باگ واقعی پیدا و رفع شد** ⚠️→✅ | ۳ ستون | تمیز بعد از رفع |

**باگ واقعی که تست پیدا کرد:** در پنل ادمین، اینپوت‌های ردیف «لینک دانلود» (`.dl-row input`) داخل یک گرید بودن، ولی مرورگر به‌صورت پیش‌فرض به `<input>` یه حداقل‌عرض داخلی (۲۱۲px) می‌ده که مستقل از تنظیمات گرید عمل می‌کنه — این باعث می‌شد در عرض ۷۶۸px، اینپوت‌ها از قاب بیرون بزنن (Overflow افقی واقعی، نه فرضی). با `min-width: 0` روی اینپوت‌ها (و همین‌طور `.field` و `.filters-bar` به‌صورت پیشگیرانه) رفع شد و با تست دوباره تأیید شد کاملاً درست شده (`scrollWidth === clientWidth` دقیقاً در هر ۴ عرض).

همچنین clearance هیرو روی صفحه‌نمایش کوتاه (۳۷۵×۶۶۷، معادل iPhone SE) مستقیماً اندازه‌گیری شد: متن هیرو ۵۷px زیر هدر شروع می‌شه (بدون همپوشانی) و دکمه‌های هیرو ۱۰۲px بالاتر از bottom-nav تموم می‌شن (بدون همپوشانی).
...
├── favorites.html            علاقه‌مندی‌های کاربر (محلی + سینک‌شده با دیتابیس برای کاربر لاگین‌کرده)
├── login.html                 ورود/ثبت‌نام بازدیدکننده (ایمیل/رمز یا گوگل)
...
│   ├── auth.js                    ورود بازدیدکننده (گوگل + ایمیل/رمز) + مدیریت پروفایل/بلاک
│   ├── login.js                    منطق صفحه ورود/ثبت‌نام اختصاصی (login.html)
│   ├── chrome.js               هدر + ناوبری پایین صفحه + لوگو + دکمه حساب کاربری
│   ├── cache.js                 کش sessionStorage با انقضا
│   ├── ui.js                     Toast، Skeleton، Favorites (+ سینک با دیتابیس)، Share، کارت فیلم، آیکون ستاره، loading state
│   ├── comments.js               نظر+امتیاز و پرسش‌وپاسخ صفحه جزئیات (نیازمند ورود)
...
### Collection: `favorites` (علاقه‌مندی‌های کاربر لاگین‌کرده، جدید)
هر سند با ID = `${uid}_${movieId}`: userId, movieId, createdAt. برای کاربر مهمان (لاگین‌نکرده) علاقه‌مندی‌ها فقط در `localStorage` مرورگر خودش می‌مونه؛ به محض ورود، لیست محلی و ابری با هم merge می‌شن.
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /admins/{uid} {
      // یک کاربر فقط می‌تونه سند خودش رو با ID مشخص بخونه (برای چک کردن اینکه
      // خودش ادمینه یا نه) — نه کل لیست رو enumerate کنه؛ write همیشه فقط از
      // کنسول فایربیس دستی انجام می‌شه.
...
**تغییر مهم نسبت به مرحله قبل:** ثبت نظر/سوال دیگه بدون لاگین ممکن نیست؛ حالا نیاز به ورود (با گوگل یا ایمیل/رمز از صفحه `login.html`) داره. نسخه‌ی قبلی Rules یک شرط سخت‌گیرانه‌تر داشت (`request.resource.data.name == request.auth.token.name`) که اسم رو دقیقاً با claim توکن گوگل مقایسه می‌کرد؛ این شرط رو حذف کردم چون برای حساب‌های ایمیل/رمز، این claim همیشه به‌موقع پر نمی‌شه و باعث می‌شد ثبت نظر/سوال با خطای permission-denied مواجه بشه (همون ارور «پرسش و پاسخ کار نمی‌کنه»). امنیت اصلی همچنان با `userId == request.auth.uid` تضمین می‌شه — یعنی کسی نمی‌تونه از طرف حساب کاربری دیگه‌ای چیزی بنویسه، فقط دیگه اسم claim دقیقاً چک نمی‌شه.

**کالکشن جدید `favorites`:** برای همگام‌سازی علاقه‌مندی‌های کاربر لاگین‌کرده بین دستگاه‌ها؛ هر سند با ID = `${uid}_${movieId}` و فیلدهای userId/movieId/createdAt. کاربر فقط به سندهای خودش (`userId == خودش`) دسترسی داره.

**درباره فعال‌سازی ورود با گوگل:** باید یک‌بار در Firebase Console → Authentication → Sign-in method → Google رو فعال کنی (اگه از قبل فعال نکرده باشی)، و همچنین Email/Password رو هم فعال کن (چون صفحه `login.html` الان امکان ثبت‌نام با ایمیل/رمز رو هم داره). اگه دکمه «ادامه با گوگل» همچنان خطا می‌ده، این‌ها رو چک کن:
- Authentication → Sign-in method → Google باید Enabled باشه.
- Authentication → Settings → Authorized domains باید دامنه‌ای که سایت روش هاست شده (مثلاً `username.github.io`) رو داشته باشه.
- خطای دقیق حالا رو صفحه لاگین نمایش داده می‌شه (نه فقط «ناموفق»)؛ اون متن رو برام بفرست اگه باز مشکل داشت.
