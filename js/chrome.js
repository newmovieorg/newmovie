...
import { googleSignIn, visitorSignOut, watchVisitor } from "./auth.js";
...
function initHeaderBehavior() {
  ...
  const isCollapsed = () => window.matchMedia("(max-width: 720px)").matches;
  const goSearch = () => {
    const term = searchInput.value.trim();
    if (term) location.href = `movies.html?q=${encodeURIComponent(term)}`;
  };
  searchBtn?.addEventListener("click", () => {
    if (isCollapsed() && !searchBox.classList.contains("expanded")) {
  ...
  searchInput?.addEventListener("blur", () => {
    if (isCollapsed() && !searchInput.value) searchBox.classList.remove("expanded");
  });
  ...
  watchVisitor((user) => {
    if (user) {
      ...
      btn.innerHTML = user.photoURL
        ...
        : USER_ICON;
      menu.innerHTML = `
        <div class="account-menu-name">${user.displayName || user.email || "کاربر"}</div>
