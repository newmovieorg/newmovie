...
function publicName(userOrComment) {
  const name = typeof userOrComment === "string"
    ? userOrComment
    : userOrComment?.displayName;
  return name && !name.includes("@") ? name : "کاربر";
}
  ...
  box.innerHTML = comments.map(c => `
    <div class="comment-item">
      <div class="comment-head">
        <strong>${escapeHTML(publicName(c.name))}</strong>
  ...
  watchVisitor((user) => {
    ...
    container.innerHTML = `
      <form class="review-form" id="commentForm">
        <div class="field"><label>ثبت نظر و امتیاز شما (به نام ${escapeHTML(publicName(user))})</label></div>
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        await addDoc(collection(db, "comments"), {
           movieId, userId: user.uid, name: publicName(user),
...
export function renderQuestions(elId, questions) {
  ...
  box.innerHTML = questions.length
    ? questions.map(q => `
      <div class="qna-item">
        <div class="qna-q">
            <strong>${escapeHTML(publicName(q.name))}</strong>
  ...
  watchVisitor((user) => {
    ...
    container.innerHTML = `
      <form class="review-form" id="questionForm">
        <div class="field"><label>سوالت رو بپرس (به نام ${escapeHTML(publicName(user))})</label></div>
